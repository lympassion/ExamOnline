...
| qu_opa     | VARCHAR(255) | 如果是选择则代表A选项 | 否       | 根据具体题型决定是否为空                                     |
...
| qu_score | INTEGER      | 教师评分               | 否       | <= 对应试题分值         |
=======
>>>>>>> 9f415d3b8e5be8fc970a6d07cef44131ee684cbb
>>>>>>> Stashed changes
...
| ep_id        | VARCHAR(10)  | 对应试卷编号   | 是       | 表exampaper的外键           |
| ans_rec      | JSON         | 答题记录       | 是       | score <= 对应试题分值       |
| total_score  | INTEGER      | 考试得分       | 是       |                             |

其中考试记录ans_rec字段的存储类型为JSON，key为试题编号，value为一个嵌套的JSON，其中有两个key，分别为学生回答ans和评分score。

由于主观题需要教师评分，所以score默认为0。
...

{
    ...
    "6": {"ans": "ABD", "score": 0},
    "11": {"ans": "面向对象", "score": 5},
    "12": {"ans": "这道题我不会，老师多少给一点分吧……", "score": 0}
}
```
