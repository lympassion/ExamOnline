...
public class UserServiceImpl implements UserService {
    ...
    public MyUser checkUser(MyUser user) {
        if(user.getRole() == MyUser.ROLE_STUDENT) {
        ...
        if(user.getRole() == MyUser.ROLE_TEACHER){
        ...
        if(user.getRole() == MyUser.ROLE_ADMIN){
    ...
    public MyUser checkUserById(int id) {
        if(String.valueOf(id).substring(0,1).equals(String.valueOf(MyUser.ROLE_STUDENT))) {
            Student student = stuMapper.selectById(id);
            return new MyUser(student.getStudentId(),student.getStudentPassword());
        }
        if(String.valueOf(id).substring(0,1).equals(String.valueOf(MyUser.ROLE_TEACHER))){
            Teacher teacher = teacherMapper.SelectTeacherById(id);
            return new MyUser(teacher.getTeacherId(),teacher.getTeacherPassword());
        }if(String.valueOf(id).substring(0,1).equals(String.valueOf(MyUser.ROLE_ADMIN))){
